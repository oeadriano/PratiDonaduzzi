...
WITH
  ...
  SELECT DISTINCT 
    ...
    FROM w_lista_base AS vbrk
         ...
         LEFT JOIN `sap_raw.vbak`     AS vbak     ON vbak.mandt = ztbsf004.mandt AND vbak.vbeln = ztbsf004.aubel
   ...
   WHERE vbrk.origem = 'S'
      and vbkd.fkdat >= date_sub(current_date, interval 1 day)
      and vbak.erdat >= date_sub(current_date, interval 1 day)

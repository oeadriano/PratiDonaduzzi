Backup antes da entrada do SQL "dados-prod.sap_view.VB_TR_LOJAS_PRECOS-260923-NEW.sql"
--PROD														
...
with 
    ...
    w_combo as (        
		SELECT 
            PRODUTO, ID as FILTRO_CONTEXTO_ID, ID as FILTRO_CONTEXTO_NOME, '' as VKORG                    
        FROM
        ...
        --PROD
        EXTERNAL_QUERY("projects/dados-prod/locations/southamerica-east1/connections/cimed-postgres ", "SELECT DISTINCT 'Combo' AS id, p.produto__c as produto FROM  SF.CADASTROCOMBO__C AS C JOIN  SF.PRODUTOSCOMBO__C AS P  ON P.CADASTROCOMBO__C = C.ID WHERE  c.tipocombo__c ILIKE 'Combo%'  AND c.ativo__c = 'true';")
    )    
    , w_material as
    (   
        select
    ...
    (
        SELECT DISTINCT
            ...
            CROSSDOCKING
        FROM
            `dados-prod.sap.VH_MD_LIFNR_CLIENTE` L
        JOIN 
            `dados-prod.sap.VH_TR_CADASTRO_CLIENTES` C
            on C.codigo = L.kunnr
        JOIN
            w_empresa e
            on e.vkorg = L.vkorg
...
group by
    ...
    V_ZFAT, V_ZSTA, ZPFA, ZPMC, COMBO, mat.url, cont.FILTRO_CONTEXTO_NOME, cont.FILTRO_CONTEXTO_ID, positivado, 

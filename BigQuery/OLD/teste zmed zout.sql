...
with w_ydsd044 as
		...
		select
			...
			linha, status, c_controlado, produto_hierarquia, substring(hierarquia, 4, 3) as cod_hierarquia, hierarquia, CASE
				...
				WHEN GRP_MERCADORIA IN ('PA15')
					THEN 'Terceiros'
					ELSE GRP_MERCADORIA
			END AS grp_mercadoria, fabricante, caixa_padrao, ipi,farm_popular, prod_marca, prod_classei, prod_fator, ncm, grpmercexterno, MENU_CATEGORIA, NUM_CATEGORIA
		from
	...
	(
		select        distinct
			k.kunnr, p.matnr, max(p.erdat) as erdat, round( (sum(p.netwr) + sum(p.MWSBP)) / sum(p.KWMENG), 2) as vlr_unit, sum(p.KWMENG) as ult_qde
		from
	...
	(
	SELECT
			produto, NOME as FILTRO_CONTEXTO_NOME, id as FILTRO_CONTEXTO_ID
		FROM
			EXTERNAL_QUERY("projects/dados-dev/locations/us/connections/cimed-postgres-us", "SELECT * from sf.view_prd_destaque")
	)
select
	MAT.CODIGOBARRAS, MAT.DESCRICAO_MATERIAL, MAT.PRINCIPIOATIVO, MAT.GENERICO, MAT.CODIGO_MS, MAT.LINHA, MAT.STATUS, MAT.C_CONTROLADO,
    ...
    round(V_ZFAT,2) AS V_ZFAT, 
    round(V_ZSTA,2) AS V_ZSTA, 
	round(ZPFA,2) AS ZPFA, 
	round(ZPMC,2) AS ZPMC, 
	
	round(SUM(VALOR_P),2) AS VALOR_P, round(SUM(VALOR_M),2) AS VALOR_M, round(SUM(VALOR_G),2) AS VALOR_G, 
	round(SUM(VALOR_G1),2) AS VALOR_G1, round(SUM(VALOR_G2),2) AS VALOR_G2, SUM(QDE_P) AS QDE_P, SUM(QDE_M) AS QDE_M, SUM(QDE_G) AS QDE_G, SUM(QDE_G1) AS QDE_G1, 
	SUM(QDE_G2) AS QDE_G2, SUM(COMIS_P) AS COMIS_P, SUM(COMIS_M) AS COMIS_M, SUM(COMIS_G) AS COMIS_G, SUM(COMIS_G1) AS COMIS_G1, SUM(COMIS_G2) AS COMIS_G2, 
	'https://storage.googleapis.com/gc-conteudo/'
		||substring(MAT.hierarquia, 1, 3)
		|| '/'
		||substring(MAT.hierarquia, 4, 3)
		|| '/'
		||substring(MAT.hierarquia, 5, 3)
		||'/'
		...
		||'/'
		||substring(P.produto,13,6)
		||'-I.png' as IMAGEM, 'https://storage.googleapis.com/gc-conteudo/'
		||substring(MAT.hierarquia, 1, 3)
		|| '/'
		||substring(MAT.hierarquia, 4, 3)
		|| '/'
		...
		||'/'
		||substring(P.produto,13,6)
		||'-B.pdf' as BULA, 'https://storage.googleapis.com/gc-conteudo/'
		||substring(MAT.hierarquia, 1, 3)
		|| '/'
		||substring(MAT.hierarquia, 4, 3)
		|| '/'
		...
		||'/'
		||substring(P.produto,13,6)
		||'-F.pdf' as FICHA, COALESCE(VC.VALOR, 0) AS VALOR_VC, CASE
	...
	cast(p.DISPONIVEL as integer) AS DISPONIVEL, '' as FILTRO_CONTEXTO, '' as NUM_FILTRO, coalesce(cont.FILTRO_CONTEXTO_ID, '') as FILTRO_CONTEXTO_ID, 
	coalesce(cont.FILTRO_CONTEXTO_NOME, '') as FILTRO_CONTEXTO_NOME, 250 as PEDIDO_MINIMO, 'P' as TIPO_P, 'M' as TIPO_M, 'G' as TIPO_G,
	...
	join
		...
		(
			with w_gama_produto as
				(
					select
						c.cod_gama, c.matnr, 
						case 
							when M.grp_mercadoria in ('PA11', 'PA12', 'PA13') AND m.grp_mercadoria not in ('PA07', 'PA08', 'PA09') then 'ZOUT'
							else 'ZMED'
						end as material_zmed_zout, 
						case 
							when M.grp_mercadoria in ('PA07', 'PA08', 'PA09') then 'S'
							else 'N'
			...
			SELECT   DISTINCT
				...
				lf.kunnr, b.descricao, b.vtweg, a.werks, y94.func_par, c.matnr
			FROM
			...
			WHERE
				...
				and a.werks <> '1100'							
				and 
					(c.material_zmed_zout = 
						case 
							when lf.cliente_zmed_zout = 'ZOUT' then 'ZOUT' 
							else c.material_zmed_zout 
						end
								
					or 
						c.material_controlado = lf.cliente_vende_controlado
					)
		)
		, w_zco2 as
		(
...
group by
    ...
    V_ZFAT, V_ZSTA, ZPFA, ZPMC
	
	
/*

0001001064 - 0000603978 - ZMED - 158 - fly 450
0001007223 - 0000604211 - ZOUT - hibrida - 238
0001018304 - 0000604410 - ZOUT - hibrida - 238
0001043031 - 0000700812 -  - ZOUT - now - 180p
0001074232 - 0000603077 - ZOUT - 159 - now - 180

SELECT v.cod_gama, v.descricao, m.grp_mercadoria, *
FROM `dados-dev.visoes_cimed_tech.CV_A996_LOJAS_GERAL_teste_zmed`v 
join `dados-dev.raw_cimed_tech.CV_CADASTRO_MATERIAL_T` m on substring(m.codigo, 13,6) = v.produto
Where    
    lifnr = '0000603077'
    and kunnr = '0001074232'
	

SELECT v.cod_gama, v.descricao, m.grp_mercadoria, *
FROM `dados-dev.visoes_cimed_tech.CV_A996_LOJAS_GERAL`v 
join `dados-dev.raw_cimed_tech.CV_CADASTRO_MATERIAL_T` m on substring(m.codigo, 13,6) = v.produto
Where    
    lifnr = '0000603077'
    and kunnr = '0001074232'  
...
controlado
0000603978 - 0001021213 - CLI SIM
0000603978 - 0001020642 - CLI NAO
	
select L.vkorg, l.lifnr, l.kunnr, C.CONTROLADO, 
from `dados-dev.raw_cimed_tech.LIFNR_CLIENTE_T` L
join `dados-dev.raw_cimed_tech.CADASTRO_CLIENTE_T` C
on c.codigo = L.kunnr
where l.lifnr = '0000603978' and 
controlado = 'N' and l.vkorg = '3000'

select p.* , l.*
from `dados-dev.raw_cimed_tech.YDSD057` P
join `dados-dev.raw_cimed_tech.YDSD056` L
on l.cod_gama = P.COD_GAMA
where L.ativo  = 'S' and 
P.matnr in 
(
select codigo 
from `dados-dev.raw_cimed_tech.CV_CADASTRO_MATERIAL_T` M 
where M.C_CONTROLADO = 'S'
) 

...
-- PRR - 24/08/23 Alterações do foguete Amarelo - cliente e produto
...
with 
    ...
    w_cat_old AS (
        ...
        FROM 
            dados-prod.sap.TB_TR_SF_ATRIB_PRODUTO
    ),
    w_preco_geral as  
    (
    ...
    (
    SELECT 
        distinct PRODUTO, 0 AS VALOR, 0 AS SALDO, VKORG, FILTRO_CONTEXTO_ID, FILTRO_CONTEXTO_NOME, MENU_CATEGORIA, NUM_CATEGORIA
    FROM 
    ...
    w_combo as (        
		SELECT																		  
            DISTINCT PRODUTO, coalesce(FILTRO_CONTEXTO_ID,'') as FILTRO_CONTEXTO_ID, 
            coalesce(FILTRO_CONTEXTO_NOME, '') as FILTRO_CONTEXTO_NOME, VKORG        
        FROM
            dados-prod.sap.TB_TR_SF_VIEW_DESTAQUE_COMBO
    )    
    , w_material as
    (   
        select
            ...
            case 
                ...
                when MATKL in ('PA11', 'PA12', 'PA13', 'PA15') then 'ZOUT'
                else 'ZMED'
    ...
    w_loja_new as (
        ...
        with w_cat_comiss as (
            ...
            --SELECT * FROM dados-prod.sap.TB_TR_CATEGORIA_COMISSAO
        )
        SELECT 
            ...
            c_comis.CATEGORIA__C, lf.crossdocking, lf.cliente_fa, mat.produto_fa
        FROM 
		...
		/*
            (   
                ...
                OR
                (CASE 
                    WHEN lf.cliente_vende_controlado = 'S' and lf.cli_mil_pro = 'N' THEN
                        lf.cliente_zmed_zout in('ZMED', 'ZSAC', 'ZOUT', 'YB02') and mat.material_zmed_zout in ('ZOUT', 'ZMED') and coalesce(lf.BLOQUEIO, '') = '' and mat.prd_mil_pro = 'N'

                    WHEN lf.cliente_vende_controlado = 'S' and lf.cli_mil_pro = 'S' THEN
                        lf.cliente_zmed_zout in('ZMED', 'ZSAC', 'ZOUT', 'YB02') and mat.material_zmed_zout in ('ZOUT', 'ZMED') and coalesce(lf.BLOQUEIO, '') = ''

                    WHEN lf.cliente_vende_controlado = 'N' and lf.cli_mil_pro = 'N' THEN
                        lf.cliente_zmed_zout in('ZMED', 'ZSAC', 'ZOUT', 'YB02') and mat.material_zmed_zout in ('ZOUT', 'ZMED') and mat.c_controlado = 'N' and coalesce(lf.BLOQUEIO, '') = '' and mat.prd_mil_pro = 'N'

                    ELSE 
                        -- resta: lf.cliente_vende_controlado = 'N' and lf.cli_mil_pro = 'S' THEN
                        lf.cliente_zmed_zout in('ZMED', 'ZSAC', 'ZOUT', 'YB02') and mat.material_zmed_zout in ('ZOUT', 'ZMED') and mat.c_controlado = 'N' and coalesce(lf.BLOQUEIO, '') = ''
                END)
            ) 		
		*/
            (   
    ...
    (
        select 
            distinct PRODUTO, VALOR, SALDO, coalesce(VKORG, '') as VKORG, 
            coalesce(FILTRO_CONTEXTO_ID, '') as FILTRO_CONTEXTO_ID, 
            coalesce(FILTRO_CONTEXTO_NOME, '') as FILTRO_CONTEXTO_NOME
        from (
            ...
            where 
              coalesce(C.vkorg, '') = ''
            UNION ALL																				   
            -- contexto para foguete amarelo
            select 
                distinct matnr as PRODUTO, 0 AS VALOR, 0 AS SALDO, VKORG, 'Foguete Amarelo' as FILTRO_CONTEXTO_ID, 'Foguete Amarelo' as FILTRO_CONTEXTO_NOME
            from 
                w_loja_new
            where
                produto_fa = 'S' and  cliente_fa = 'S'
            order by
    ...
    select
        ...
        y44.lgort_ytri, loja.cliente_fa
    from
        w_loja_new as loja
    join
...
select
    ...
    case 
        when coalesce(cont.FILTRO_CONTEXTO_ID, '') = 'Foguete Amarelo' and P.CLIENTE_FA = 'N' then ''
        else coalesce(cont.FILTRO_CONTEXTO_ID, '')
    end as FILTRO_CONTEXTO_ID,
    case 
        when coalesce(cont.FILTRO_CONTEXTO_NOME, '') = 'Foguete Amarelo' and P.CLIENTE_FA = 'N' then ''
        else coalesce(cont.FILTRO_CONTEXTO_NOME, '')
    ...
    -- cabe uma versão da api com o tipo de documento
	  case 
        when P.cliente_fa = 'S' and mat.produto_fa = 'S' then 'ZNOR-FA'
        else 'ZNOR' 
    end AS INT_ZNOR, 
	  'ZNOR-VC' as INT_ZV12,
    case 
    ...
    P.CATEGORIA__C
    ,p.WERKS_CROSS, p.LGORT_CROSS, p.INTEGRACAO_CROSS, 20000.00 as PEDIDO_MINIMO_CROSS, 
	p.lgort_ytri as INTEGRACAO_LGORT_CROSS,
    -- AEO 28-11-23 trata flag produto_fa
    case 
      when P.CLIENTE_FA = 'N' then 'N' 
      else mat.PRODUTO_FA
    end as PRODUTO_FA,
    P.CLIENTE_FA
from
...
group by
	...
	mat.PRODUTO_FA, P.CLIENTE_FA

  )
  where 
  kunnr = '0001129004' 
  and lifnr = '0000702398' 
  and vkorg = '3300'
  and produto = '100001'
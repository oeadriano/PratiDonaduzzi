...
join
	...
	(
		...
		WHERE
			...
			and a.werks <> '1100'							
			and c.matnr in (
				case when lf.cliente_zmed_zout = 'ZMED' then (select distinct codigo from w_material)
				else (select distinct codigo from w_material where grp_mercadoria in ('PA11', 'PA12', 'PA13'))
				end )		
				or c.material_controlado = lf.cliente_vende_controlado

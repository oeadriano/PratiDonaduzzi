...
-- dados-dev.sap_view.VB_TR_LOJAS_PRECOS
with 
    w_ultima as (
        -- 
    select  
    ...
    w_cat_old AS (
        SELECT 
        --codigo_sap__c, id_categoria, categoria_produto__c FROM sap_view.TB_TR_ATRIB_PRODUTO
		--DEV
            *
        FROM 
    ...
    (
        ...
        from `dados-dev.sap.VH_TR_LOJAS_PRECO_FIXO`
        where USO = 'VI'
    ),	  
    w_empresa as    
    (
    ...
    w_combo as (        
		SELECT																		  
            PRODUTO, FILTRO_CONTEXTO_ID, FILTRO_CONTEXTO_NOME, VKORG        
        FROM
            dados-dev.sap.TB_TR_SF_VIEW_DESTAQUE_COMBO
    )    
    , w_material as
    (   
        select
    ...
    (
        ...
        FROM
          `dados-dev.sap.VH_TR_REP_CLIENTES` C
        JOIN
          w_empresa e
          on e.vkorg = C.vkorg
    ),    

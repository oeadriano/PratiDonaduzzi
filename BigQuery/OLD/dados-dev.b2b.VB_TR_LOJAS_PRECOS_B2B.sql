...
with 
    ...
    (
        with w_func_par as (
            select 'Y1' as func_par union all
            select 'X1' as func_par

        )
        --SELECT * FROM `dados-dev.sap.VH_TR_LOJAS_PRECO_GERAL` 
        SELECT 
            B.*, f.func_par, '0' as COMIS_P, '0' AS COMIS_M, '0' AS COMIS_G
        FROM 
            `dados-dev.sap.VH_TR_LOJAS_PRECO_GERAL_B2B` B
        cross join
            w_func_par as f
    ),
    w_categoria as (
    ...
    , w_contexto AS
    (
        /*-- AEO 31/01/23 - comentado pois é preciso criar outra tabela de material_contexto para b2b
		select 
    ...
    , w_pedido_minimo as (
      select 
        distinct condicao, vkorg, canal, uf_org as cliente, cliente as valor
      from 
        `dados-dev.sap.VH_TR_PED_PARC_MINIMA_CLIENTE_B2B`
      where 
        condicao = 'ZMIN'
...
select
    ...
    max(QDE_P) AS QDE_P, max(QDE_M) AS QDE_M, max(QDE_G) AS QDE_G,
    max(COMIS_P) AS COMIS_P, max(COMIS_M) AS COMIS_M, max(COMIS_G) AS COMIS_G,
    mat.URL, 
    ...
    coalesce(pmin.valor, '300') as PEDIDO_MINIMO, 'P' as TIPO_P, 'M' as TIPO_M, 'G' as TIPO_G,
...
left join
    w_pedido_minimo as pmin
    on pmin.vkorg = p.vkorg
    and pmin.canal = p.vtweg
    and pmin.cliente = p.kunnr
group by
    ...
    V_ZFAT, V_ZSTA, ZPFA, ZPMC, COMBO, mat.url, cont.FILTRO_CONTEXTO_NOME, cont.FILTRO_CONTEXTO_ID, pmin.valor
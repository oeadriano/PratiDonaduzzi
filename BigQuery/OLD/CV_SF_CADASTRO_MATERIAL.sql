...
SELECT DISTINCT
    ...
    coalesce(HIE.CLASSEI_DESCR, '') as HIERARQUIA
from 
...
left join
    ...
    and MAR.MATKL not in ('PA11', 'PA12') -- kit cosmeticos pode ter mais de registro cadastrado. nao obrigatorio.
left join
    dados-dev.visoes_auxiliares_cimed_tech.CV_DIM_CLASSIFIC_MAT CON
    on CON.MANDT = MAR.MANDT
    and CON.OBJEK = MAR.MATNR 
    --and (con.ATINN = '0000000822' OR con.ATINN = '0000000824')
...
left join 
    dados-dev.visoes_auxiliares_cimed_tech.CV_DIM_CLASSIFIC_MAT STA
    on STA.MANDT = MAR.MANDT        
    AND STA.OBJEK = MAR.MATNR
    AND STA.ATNAM = 'MAT_STATUS'    
left join 
    dados-dev.raw.MARM MAM
    on MAR.matnr = MAM.matnr 
    AND MAR.mtart IN ('FERT', 'HAWA', 'YMKT') 
    AND MAM.meinh = 'CX'    
    AND CAST(MAM.umren AS STRING) = '1'
join
    dados-dev.raw.MARC mc
    on mc.matnr = mar.matnr and mc.werks = '3000'
where
    --MAR.MATKL in (SELECT LOW FROM TVARVC WHERE NAME = 'YSD_MATKL' AND MANDT = '500')
    --and 
    MAR.MANDT = '500'
    and MAR.MSTAE in ('', 'Y5') 
    ...
    and MAK.SPRAS = 'P'
    --and est.saldo > 0
--  and mar.matnr = '000000000000100004'
order by
    MAR.MATNR;
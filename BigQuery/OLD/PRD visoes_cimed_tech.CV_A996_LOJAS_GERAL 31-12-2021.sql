with w_ydsd044 as
	...
	SELECT
		...
		FROM
			--DEV
			--EXTERNAL_QUERY("projects/dados-prod/locations/us/connections/cimed-postgres-us", "SELECT * from sf.view_prd_destaque")
			--PROD
...
select
    ...
    max(round(VALOR_P,2)) AS VALOR_P, max(round(VALOR_M,2)) AS VALOR_M, max(round(VALOR_G,2)) AS VALOR_G, max(round(VALOR_G1,2)) AS VALOR_G1, max(round(VALOR_G2,2)) AS VALOR_G2,     
	max(QDE_P) AS QDE_P, max(QDE_M) AS QDE_M, max(QDE_G) AS QDE_G, max(QDE_G1) AS QDE_G1, 
	max(QDE_G2) AS QDE_G2, max(COMIS_P) AS COMIS_P, max(COMIS_M) AS COMIS_M, max(COMIS_G) AS COMIS_G, max(COMIS_G1) AS COMIS_G1, max(COMIS_G2) AS COMIS_G2, 

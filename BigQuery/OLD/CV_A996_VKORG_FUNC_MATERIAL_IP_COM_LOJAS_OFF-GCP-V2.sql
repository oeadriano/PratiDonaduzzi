...
FROM 
	(
	SELECT
		PMG.produto, PMG.LIFNR, PMG.Kunnr, 
		PMG.perc_comis,
		...
		case
			when PMG.faixa = 0 then round(((ZFAT.VALOR / (1 - PMG.valor/100)) + ZSTA.VALOR),2)
			else 0
		end as VALOR_G,	
		case
		...
		case
			when PMG.faixa = 0 then COALESCE(ZQ.QTDMIN, 1)
			else 0
		end as QDE_G, 
...
--LEFT JOIN 
--	dados-dev.raw_cimed_tech.CV_YDSD_ATUALIZAR_ESTOQUE_T E
--	ON E.MATNR = P.PRODUTO
--		AND E.WERKS = P.WERKS	
LEFT JOIN
-- ultima compra do cliente, subir with
	(
		select 
...
join
	-- mock, vai vir no cadastro de material
	(
	select 
...
group by 
	...
	P.produto, P.LIFNR, P.kunnr, P.perc_comis, ZPFA, ZPMC, P.V_ZFAT, P.V_ZSTA, P.COD_GAMA, 

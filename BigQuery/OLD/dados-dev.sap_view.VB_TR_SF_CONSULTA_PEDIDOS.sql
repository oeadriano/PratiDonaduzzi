-- sap_view.VB_TR_SF_CONSULTA_PEDIDOS
WITH w_dash_ov_fat as 
(
  SELECT 
    DOC_VENDA as PEDIDO_SAP, DT_OV_D as DATA, DT_FATUR_D, COND_PAG, CLIENTE, 
    case
    ...
    end AS STATUS_FINAL, ORG_VENDA, CANAL_DISTR, coalesce(CT_CAMPANHA, '') as CT_CAMPANHA
    from `sap.VH_TR_DASH_MV_VISAO_OV`
    where
    ...
    --AND DOC_TIPO in ('ZNOR', 'ZGOV', 'YTRI', 'ZV12')     
    GROUP BY
		DOC_VENDA, DT_OV_D, DT_FATUR_D, COND_PAG, CLIENTE, RAZAO_SOCIAL, COCKPIT, RAZAO_SOCIAL,
  ...
  SELECT 
    ...
    case
      when COCKPIT = 'Faturamento' then
        round(SUM(VALOR_NF),2)
      else
        round(SUM(DEVOLUCAO_NF) * -1,2) 
    end AS VALOR, 
    CNPJCLIENTE AS CNPJ, 
    RAZAO_SOCIAL,	VENDEDOR, PEDIDO_PORTAL, 
    UNIX_MILLIS(timestamp(etl_venda)) as DATAHORA_TIMESTAMP, 
...
w_ped_aux as (
    SELECT 
        ...
        -- cast(REPLACE(REPLACE(cabecalho.pedido, 'CT-', ''), '-B', '') as INT64) as timestamp_pedido, cabecalho.razao_social, 
        -- AEO 14/02/23        
        cast(REPLACE(REPLACE(REPLACE(cabecalho.pedido, 'CT-', ''), '-B', ''), 'B2B', '') as INT64) as timestamp_pedido, cabecalho.razao_social,  
        cabecalho.orgvendas, cabecalho.canal, integrador as ERNAM,t.parcelas as qde_parcelas, 'Integrado' as status_final,         
        t.parcelas|| '_' || t.zterm || '_' || t.vtext as id_unico, etl_timestamp, coalesce(cabecalho.campanhaId, '') as CT_CAMPANHA
    FROM 
        `raw_cimed_tech.ct_pedidos_auxiliar`, UNNEST(itens) as itens
    LEFT JOIN 
        w_TVZBT AS T
    ...
    group by 
		...
		integrador, t.parcelas, etl_timestamp, cabecalho.campanhaId
),
w_cancelado as (
  select 
    * 
  from 
    `sap.VH_TR_PEDIDOS_CLIENTE_CANCELADO` C 
  where 
...
w_dash as (
	SELECT 
		D.PEDIDO_SAP, D.DATA, DT_FATUR_D,
...
SELECT
  Data_OV__c, Data_Fatur__c, Pedido_Portal__c, Valor_do_pedido_original__c, 
  valor_faturado,
  status,
  CASE
    WHEN Pedido_Portal__c LIKE 'B2B%' AND STATUS = 'Aberto'    THEN 'Pedido recebido'
    WHEN Pedido_Portal__c LIKE 'B2B%' AND STATUS = 'Integrado' THEN 'Em análise'
    WHEN Pedido_Portal__c LIKE 'B2B%' AND STATUS = 'Separação' THEN 'Pedido confirmado'
    WHEN Pedido_Portal__c LIKE 'B2B%' AND STATUS = 'Entrega'   THEN 'Faturado'
    ELSE status
  END AS status_b2b,
  status_final, motivo_cancelamento, Codigo_Pedido_SAP__c, Mensagem_Integracao__c,
  Codigo_Condicao_Pagamento__c, Codigo_SAP__c, Organizacao_Vendas__c, Canal__c,
  Codigo_Vendedor_SAP__c, desdobramento, parcelas, Ernam__c, Id_unico__c,
  Data_Hora_Pedido, ETL_VENDA, ETL_REMESSA, ETL_FATURAMENTO, LAST_UPDATE,
  tipo, CT_CAMPANHA
 FROM 
(
  select 
      aux.DATA AS Data_OV__c, '' as Data_Fatur__c, aux.PEDIDO_PORTAL as Pedido_Portal__c,     
      ...
      replace(substring(cast(TIMESTAMP_MILLIS(aux.timestamp_pedido) as STRING), 1, 10)||'T'||substring(cast(TIMESTAMP_MILLIS(aux.timestamp_pedido) as STRING), 12, 10)||'Z','+0', '') as ETL_VENDA, 
      '' as ETL_REMESSA,     
      '' as ETL_FATURAMENTO,     
      cast(SUBSTRING(cast(TIMESTAMP_MILLIS(aux.timestamp_pedido) as STRING), 1, 19) as timestamp)AS LAST_UPDATE,
      'w_ped_aux' as tipo, aux.CT_CAMPANHA
  from 
      w_ped_aux as aux
  LEFT JOIN 
    w_pgsl_status as pg
  ...
  select 
      cast(D.DATA as string) as Data_OV__c, cast(D.DT_FATUR_D as string) as Data_Fatur__c, D.PEDIDO_PORTAL as Pedido_Portal__c, 
      ...
      replace(substring(cast(TIMESTAMP_MILLIS(D.DATAHORA_TIMESTAMP) AS STRING), 1, 10)||'T'||substring(cast(TIMESTAMP_MILLIS(D.DATAHORA_TIMESTAMP) AS STRING), 12, 10)||'Z', '+', '.') as Data_Hora_Pedido, 

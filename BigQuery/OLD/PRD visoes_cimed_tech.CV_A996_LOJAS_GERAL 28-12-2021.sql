-- teste para substituir `dados-prod.visoes_cimed_tech.CV_A996_LOJAS_GERAL`
with w_ydsd044 as
	(
		select distinct
			werks, lgort, 'L'||meses as meses
		from
			`dados-prod.raw_cimed_tech.YDSD044`
	)
	, w_validade_curta as
	(
		SELECT
		...
		FROM
			dados-prod.visoes_auxiliares_cimed_tech.CV_VIEW_VALIDADE_CURTA
	)
	, w_material as
	(
		select
			...
			linha, status, c_controlado, produto_hierarquia, substring(hierarquia, 4, 3) as cod_hierarquia, hierarquia, CASE
				...
				WHEN GRP_MERCADORIA IN ('PA15')
					THEN 'Terceiros'
					ELSE GRP_MERCADORIA
			END AS grp_mercadoria, fabricante, caixa_padrao, ipi,farm_popular, prod_marca, prod_classei, prod_fator, ncm, grpmercexterno, 
			MENU_CATEGORIA, NUM_CATEGORIA, COMBO
		from
			dados-prod.raw_cimed_tech.CV_CADASTRO_MATERIAL_T
	)
	, ultima_compra as
	(
		select        distinct
		...
		from
			`dados-prod.raw.VBAK` as k
			join
				`dados-prod.raw.VBAP` as p
				on
					p.vbeln = k.vbeln
		where
...
select
	...
	'https://storage.googleapis.com/gc-conteudo-prod/'
		...
		||'/'
		||substring(P.produto,13,6)
		||'-I.png' as IMAGEM, 'https://storage.googleapis.com/gc-conteudo-prod/'
		||substring(MAT.hierarquia, 1, 3)
		|| '/'
		||substring(MAT.hierarquia, 4, 3)
		|| '/'
		...
		||'/'
		||substring(P.produto,13,6)
		||'-B.pdf' as BULA, 'https://storage.googleapis.com/gc-conteudo-prod/'
		||substring(MAT.hierarquia, 1, 3)
		|| '/'
		||substring(MAT.hierarquia, 4, 3)
		|| '/'
	...
	250 as PEDIDO_MINIMO, 'P' as TIPO_P, 'M' as TIPO_M, 'G' as TIPO_G,
...
from
	...
	join
		...
		(
			with w_gama_produto as
				(
					...
					from
						dados-prod.raw_cimed_tech.YDSD057 AS c
					join 
						`dados-prod.raw_cimed_tech.CV_CADASTRO_MATERIAL_T` M
						on m.codigo = c.matnr
					where
						c.ativo = 'S' and
				...
				(
					...
					JOIN 
						`dados-prod.raw_cimed_tech.CADASTRO_CLIENTE_T` C
						on C.codigo = L.kunnr
				)
			...
			FROM
				dados-prod.raw_cimed_tech.YDSD225 AS a
			JOIN
				dados-prod.raw_cimed_tech.YDSD056 AS b
		...
		(
			...
			FROM
				dados-prod.raw.KONP AS a
				JOIN
					dados-prod.raw.A937 AS b
					ON
						a.knumh = b.knumh
			WHERE
		...
		(
			...
			FROM
				dados-prod.raw.KONP AS a
				left JOIN
					dados-prod.raw_cimed_tech.A508 AS b
					ON
						a.knumh = b.knumh
			WHERE
				a.loevm_ko <> 'X' -- marcado para exclusão
			...
			with w_konp as
				...
				(
					select
						kbetr, kstbm, knumh
					from
						dados-prod.raw_cimed_tech.KONM
				)
				, w_a709 as
				(
					SELECT
					...
					FROM
						w_konp AS a
						JOIN
							dados-prod.raw_cimed_tech.A709 AS b
							ON
								a.knumh = b.knumh
						JOIN
							w_konm AS c
			...
			from
				(
					...
					FROM
						dados-prod.raw.KONP AS a
						left JOIN
							dados-prod.raw_cimed_tech.A954 AS b
							ON
								a.knumh = b.knumh
					WHERE
						a.loevm_ko <> 'X' -- marcado para exclusão
				...
				q
				join
					dados-prod.raw_cimed_tech.J_1BTXIC1 J
					ON
				...
				JOIN
					(
						...
						FROM
							dados-prod.raw_cimed_tech.J_1BTXIC1
						WHERE
							SHIPFROM       =SHIPTO
							AND specf_rate = 0
						GROUP BY
			...
			from
				(
					...
					FROM
						dados-prod.raw.KONP AS a
						left JOIN
							dados-prod.raw_cimed_tech.A955 AS b
							ON
								a.knumh = b.knumh
					WHERE
						a.loevm_ko <> 'X' -- marcado para exclusão

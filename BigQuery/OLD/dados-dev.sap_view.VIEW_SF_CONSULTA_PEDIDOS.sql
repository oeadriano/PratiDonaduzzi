-- dados-dev.sap_view.VIEW_SF_CONSULTA_PEDIDOS
...
(
  SELECT 
    ...
    UNIX_MILLIS(timestamp(etl_venda)) as DATAHORA_TIMESTAMP, ERNAM, CENTRO, ETL_VENDA, ETL_REMESSA, ETL_FATURAMENTO,
    case		
      when COCKPIT = 'Faturamento' then 'Faturado'
    ...
    from `dados-dev.sap.VH_TR_DASH_MV_VISAO_OV`
    where
  ...
  SELECT 
    ...
    CNPJCLIENTE AS CNPJ, 
    RAZAO_SOCIAL,	VENDEDOR, PEDIDO_PORTAL, 
    UNIX_MILLIS(timestamp(etl_venda)) as DATAHORA_TIMESTAMP, ERNAM, CENTRO, ETL_VENDA, ETL_REMESSA, 
...
w_ped_aux as (
    SELECT 
        ...
        cabecalho.orgvendas, cabecalho.canal, 
        --AEO 25.05.22 por padrao, integrador vai ser o usuario SAP
        'RFC_DIGIBEE' as ERNAM,t.parcelas as qde_parcelas, 
        case 
          when coalesce(pg.pedido_sap, '') = '' then 'Integrado' 
    ...
    group by 
		...
		integrador, t.parcelas, etl_timestamp.pg.pedido_sap, pg.status, pg.status_final
),
w_cancelado as (
  select 
    * 
...
select 
    ...
    0 as valor_faturado, 
    aux.STATUS, aux.status_final, '' as motivo_cancelamento, 

...
with 
    ...
    from 
        `dados-dev.sap.VH_TR_MAT_ULTIMA_COMPRA`
    ),
    ...
    (
        ...
        join 
			...
			and USO <> 'VI' -- somente BU 01 e 02 e 99			
    ),
    w_preco_fixo_vkorg as  
    (
        SELECT 
            distinct fx.vkorg, fx.matnr, fx.valor, lf.func_par, lf.kunnr as cliente, fx.V_ZFAT, fx.V_ZSTA, fx.ZPFA, fx.ZPMC, fx.P_COMIS
        FROM
            `dados-dev.sap.VH_TR_LOJAS_PRECO_FIXO_VKORG` fx
        join 
            `dados-dev.sap.VH_MD_LOJAS_LIFNR` lf
            on lf.vkorg = cast(fx.vkorg as string)
    ),    
    w_categoria as (
        select 
    ...
    (   
        select
            ...
            CASE
                ...
                WHEN MATKL IN ('MK02', 'MK03') THEN 'Marketing'	-- MSA			
                ELSE MATKL
            ...
            case 
                ...
                when MATKL in ('MK02', 'MK03') then 'ZMKT'    -- MSA            				
                else 'ZMED'
        ...
        where
            ...
            AND mtart IN ('FERT', 'HAWA', 'YMKT', 'NORM', 'YREV', 'HIBE') -- MSA
    ),
    w_lif_cli_new as 
    (
        SELECT DISTINCT
            ...
            case 
              when C.cliente_rca = '01' then 'ZMKT'
              else C.GRUPO_CONTAS 
            end as cliente_zmed_zout, 
    ...
    w_loja_new as (
        ...
        with w_cat_comiss as (
            SELECT * FROM `dados-dev.sf_view.VSF_CATEGORIA_COMISSAO
		)			
    SELECT 
    ...
    WHERE 
		-- cliente com cliente_rca vazio
        (
            (lf.cliente_zmed_zout = 'ZMKT' and mat.material_zmed_zout ='ZMKT')
            OR
...
select
    ...
    ZPFA, 
    ZPMC,
    max(FORMAT('%.2f',VALOR_F)) AS VALOR_F, 	
    max(QDE_F) AS QDE_F,    
    0 as COMIS_F, -- BU 01 e 02 não tem comissão
    mat.URL, 
...
join
    ...
    select
        ...
        case
            when coalesce(fx.valor, '') = '' then cast(zco2.VALOR as numeric) 
            else cast(fx.VALOR as numeric) 
            end as VALOR_F, 
        --cast(fxv.VALOR as numeric) as VALOR_Ffxv,         
        1 as QDE_F, 
        zco2.P_COMIS AS COMIS_F,        
        VC.VALOR AS VALOR_VC, vc.SALDO as QDE_VC, '0' as disponivel, '0' as transito, y44.meses, y44.lgort, 
        loja.CATEGORIA__C
    from
        w_loja_new as loja
    join
        w_preco_fixo_vkorg as zco2
    ...
    left join
        w_preco_fixo as fx
        on fx.vkorg        = zco2.vkorg
            and fx.cliente = zco2.cliente
            and fx.matnr   = zco2.matnr										   
    ) P
...
group by
    ...
    V_ZFAT, V_ZSTA, ZPFA, ZPMC, COMBO, mat.url, positivado, 
    P.CATEGORIA__C